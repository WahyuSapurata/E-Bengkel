...
@push('scripts')
    <script>
        ...
        $('#form').on('submit', function(e) {
            ...
            let updateUrl = `{{ route('outlet.po-outlet-update', ':uuid') }}`;
            updateUrl = updateUrl.replace(':uuid', uuid);

            let url = uuid ? updateUrl :
                `{{ route('outlet.po-outlet-store') }}`;
        ...
        $('#dataTables').on('click', '.delete', function() {
            let uuid = $(this).data('uuid');
            let deleteUrl = `{{ route('outlet.po-outlet-delete', ':uuid') }}`;
        ...
        const initDatatable = () => {
            ...
            $('#dataTables').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('outlet.po-outlet-get') }}",
                columns: [{
                        data: null,
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <span class="badge text-uppercase bg-${data === 'aprove' ? 'success' : 'danger'}">
                                    ${data}
                                </span>
                            `;
                        }
