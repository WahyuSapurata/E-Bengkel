                                ...
                                <table style="width: 100%" id="dataTables" class="table table-hover mb-0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-capitalize">tanggal transfer</th>
...
@push('scripts')
    <script>
        ...
        $('#form').on('submit', function(e) {
            ...
            let updateUrl = `{{ route('outlet.transfer-update', ':uuid') }}`;
            updateUrl = updateUrl.replace(':uuid', uuid);

            let url = uuid ? updateUrl :
                `{{ route('outlet.transfer-store') }}`;
        ...
        $('#dataTables').on('click', '.edit', function() {
            ...
            let editUrl = `{{ route('outlet.transfer-edit', ':uuid') }}`;
        ...
        $('#dataTables').on('click', '.delete', function() {
            let uuid = $(this).data('uuid');
            let deleteUrl = `{{ route('outlet.transfer-delete', ':uuid') }}`;
        ...
        const initDatatable = () => {
            ...
            $('#dataTables').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('outlet.transfer-get') }}",
                columns: [{
                        data: null,
