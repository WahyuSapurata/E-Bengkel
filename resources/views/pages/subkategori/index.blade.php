                                ...
                                <table style="width: 100%" id="dataTables" class="table table-hover mb-0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-capitalize">Nama sub Kategori</th>
...
@push('scripts')
    <script>
        ...
        $('#form').on('submit', function(e) {
            ...
            let updateUrl = `{{ route('superadmin.subkategori-update', ':uuid') }}`;
            updateUrl = updateUrl.replace(':uuid', uuid);

            let url = uuid ? updateUrl :
                `{{ route('superadmin.subkategori-store') }}`;
        ...
        $('#dataTables').on('click', '.edit', function() {
            ...
            let editUrl = `{{ route('superadmin.subkategori-edit', ':uuid') }}`;
        ...
        $('#dataTables').on('click', '.delete', function() {
            let uuid = $(this).data('uuid');
            let deleteUrl = `{{ route('superadmin.subkategori-delete', ':uuid') }}`;
        ...
        const initDatatable = () => {
            ...
            $('#dataTables').DataTable({
                ...
                processing: true,
                serverSide: true,
                ajax: "{{ route('superadmin.subkategori-get') }}",
                columns: [{
                        data: null,
